...
from CarWash.apps.users.car_detection.pyimagesearch.centroidtracker import CentroidTracker
from CarWash.apps.users.car_detection.pyimagesearch.trackableobject import TrackableObject
import numpy as np
import imutils
import dlib
import cv2
...
# Функция производит распознавание типа автомобиля на видео.
# Вокруг автомобиля рисуется бокс с соответствующим классом
# Функция возвращает обработанный кадр
class CarDetector():
    ...
    # Основная функция по распознаванию авто
    def detect_cars(self):
        print("\n[INFO] START DETECTING VEHICLES...\n")
        car_info = None
        while True:
            ...
            if self.totalFrames % self.skip_frames == 0:
                ...
                blob = cv2.dnn.blobFromImage(frame, 0.00392, (self.inpWidth, self.inpHeight), (0, 0, 0), True, crop=False)
            ...
            # если же кадр не явялется N-ым, то необходимо работать с массивом сформированных ранее трекеров, а не боксов
                ...
                for tracker, class_id in zip(trackers, class_ids):
                    ...
                    # рисую бокс
                    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (255, 0, 0), 2)
                    cv2.putText(frame, self.CLASSES[class_id], (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            ...
            if self.total > self.old_total:
                ...
                # кроме того, если в кадре появился новый автомобиль, то сохраняю кадр с ним в папку temp, чтобы распознать его номер
                cv2.imwrite(settings.STATICFILES_DIR + "/temp/detect_LP.jpg", frame)
